...
let state = {
  ...
  categories: {
    ...
    expense: ['🍔 Makanan','🚗 Transport','🏠 Rumah','💊 Kesehatan','👕 Belanja','🎮 Hiburan','📚 Pendidikan','💡 Utilitas','💳 Cicilan','🔧 Lainnya']
  },
  currentType: 'income',
  selectedCat: '',
...
function skipSetup() {
  ...
  showToast('ℹ️ Mode lokal – data tidak tersimpan ke Sheets');
}
...
function switchType(type) { state.currentType = type; state.selectedCat = ''; updateTypeToggle(); renderCatGrid(); }

function updateTypeToggle() {
  const inc = document.getElementById('toggle-income');
  const exp = document.getElementById('toggle-expense');
  document.getElementById('modal-title').textContent = state.currentType === 'income' ? 'Tambah Pemasukan' : 'Tambah Pengeluaran';
  inc.className = 'type-btn' + (state.currentType === 'income'  ? ' active-income'  : '');
  exp.className = 'type-btn' + (state.currentType === 'expense' ? ' active-expense' : '');
}

function renderCatGrid() {
  document.getElementById('cat-grid').innerHTML = state.categories[state.currentType]
    .map(c => `<div class="cat-chip ${state.selectedCat===c?'selected':''}" onclick="selectCat('${c.replace(/'/g,"&#39;")}')">${c}</div>`)
    .join('');
}
function selectCat(cat) { state.selectedCat = cat; renderCatGrid(); }
...
function saveTransaction() {
  ...
  const tx = { id: Date.now().toString(), type: state.currentType, amount, desc, category: state.selectedCat, date, note, synced: false };
...
function refreshHome() {
  ...
  txList.innerHTML = recent.length === 0
  ...
  return `
    <div class="tx-item">
      ...
      <div class="tx-info">
        ...
        <div class="tx-meta">${catName} · ${fmtDate(t.date)}</div>
      </div>
      <div>
...
function renderBudgetFull() {
  const txs  = txForMonth(state.currentMonth, state.currentYear);
  const list = document.getElementById('budget-list-full');
  list.innerHTML = state.budgets.length === 0
        ...
        return `<div class="budget-item">
          <div class="budget-header">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <div class="budget-nums">${fmt(spent)} / <strong>${fmt(b.amount)}</strong></div>
              <button class="delete-btn" onclick="deleteBudget('${b.category.replace(/'/g,"&#39;")}')">✕</button>
            </div>
          </div>
          <div class="progress-track"><div class="progress-fill" style="width:${pct}%;background:${color}"></div></div>
  ...
  sel.innerHTML = '<option value="">Pilih kategori</option>' +
    state.categories.expense.map(c=>`<option value="${c}">${c}</option>`).join('');
}
...
function exportCSV() {
  const rows = [['ID','Tanggal','Tipe','Kategori','Deskripsi','Jumlah','Catatan']];
  state.transactions.forEach(t => rows.push([t.id,t.date,t.type,t.category,t.desc,t.amount,t.note||'']));
...
// ══════════════════════════════════════════════════════
const CHART_COLORS = ['#58A6FF','#3FB950','#D29922','#BC8CFF','#79C0FF','#56D364','#E3B341','#F85149','#FFA657','#FF7B72'];
...
function renderBarChart() {
  ...
  const groupW = cW/n;
  const bW = Math.min(groupW*0.32, 18);
  ...
  for(let i=0;i<=4;i++){
    ...
    ctx.font=`${Math.round(10)}px monospace`;
    ctx.textAlign='right';
...
function renderDonutChart() {
  ...
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const SIZE = Math.min(canvas.parentElement.clientWidth - 32, 220);
  const dpr  = window.devicePixelRatio || 1;
  ...
  if (total === 0) {
    ...
    ctx.fillStyle='rgba(48,54,61,0.5)'; ctx.fill();
    ctx.beginPath(); ctx.arc(cx,cy,ri,0,Math.PI*2);
    ctx.fillStyle='#161B22'; ctx.fill();
    ctx.fillStyle='#8B949E'; ctx.font=`11px monospace`; ctx.textAlign='center';
    ctx.fillText('Belum ada', cx, cy-5);
    ctx.fillText('pengeluaran', cx, cy+10);
    legend.innerHTML='';
    return;
  }
